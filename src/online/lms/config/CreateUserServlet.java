...
public class CreateUserServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean status = admin.createUser(user);
		RequestDispatcher dispatcherCreate = request.getRequestDispatcher("/Serverpages/index.jsp?page=2&status=fail");
		if (status) {
			response.sendRedirect("/Online_LMS/Serverpages/index.jsp?page=3&user_added="+username);
		} else {
